module RippleCarryAdderTests exposing (andGateTests, fullAdderTests, halfAdderTest, inverterTests, rippleCarryAdderProperty1, rippleCarryAdderProperty3, rippleCarryAdderTests)
...
inverterTests =
    describe "Inverter"
        [ test "output is 1 when input is 0" <|
            \_ ->
                inverter 0
                    |> Expect.equal 1
...
rippleCarryAdderProperty1 =
    describe "carry-out's relationship with most significan digits"
        [ fuzz3
            (list (intRange 0 1))
            (list (intRange 0 1))
            (intRange 0 1)
        ...
        [ fuzz3
          ...
          <|
            \list1 list2 carryIn ->
                let
                    firstInput =
                        binaryWithLeastSignificantToZero list1

                    secondInput =
                        binaryWithLeastSignificantToZero list2

                    binaryWithLeastSignificantToZero digitsList =
                        digitsList
                            |> List.take 3
                            |> (::) 0
                            |> List.reverse
